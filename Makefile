...
OBJ = cmdline.o ctrlfunc.o minIni.o readcfg.o myexec.o gen_json_4interfaces.o certio.o
...
all-simul: mashctld.simul webdata

# Use this to generate a simulation only binary without 1-wire support
mashctld.simul mashctld-simul.o: CFLAGS += -D NOSENSACT
mashctld.simul mashctld-simul.o: LDLIBS = -lmicrohttpd -lmagic -lrt -rdynamic -lmnl

mashctld-simul.o: mashctld.c
	$(CC) $(CFLAGS) -c $< -o $@

mashctld.simul: mashctld-simul.o $(OBJ)
	$(CC) -o $@ $(CFLAGS) mashctld-simul.o $(OBJ) $(LDLIBS)

mashctld: mashctld.o $(OBJ)
	$(CC) -o $@ $(CFLAGS) mashctld.o $(OBJ) $(LDLIBS)
...
clean:
	rm -f *.o *~ mashctld mashctld.simul ifinfo find_ow_devs gpio_buzzer
...
install-simul: debian/copyright mashctld.simul
	cd webdata/; make
	mkdir -p $(DESTDIR)/share/web20mash/images
	mkdir -p $(DESTDIR)/share/web20mash/js
	mkdir -p $(DESTDIR)/share/web20mash/xslt
	...
	sed -e 's;^webroot.*;webroot = $(PREFIX)/share/web20mash;' \
		-e 's;^plugin_dir.*;plugin_dir= $(PREFIX)/lib/web20mash/plugins;' -e 's;^port.*;port = 80;' mashctld.conf.sample >$(CFDIR)/mashctld.conf
	install -m 755 mashctld.simul $(DESTDIR)/bin/mashctld
	install -m 755 webmash.service $(SERVICEDIR)
	chmod 755 $(DESTDIR)/share/web20mash $(DESTDIR)/share/web20mash/images $(DESTDIR)/share/web20mash/js $(DESTDIR)/share/web20mash/css
	chmod 644 $(DESTDIR)/share/web20mash/*/*
